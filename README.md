...
| 2025/07/16 | 今村  | 「エラーコード」節にて、E01系を追加                                                                                      |
| 2025/07/25 | 今村  | 「エラーコード」節を「エラー一覧」節に更新<br> 「.envファイル」節の「SUBMIT_FILE_NAME」を「SERVER_SUBMIT_FILE_NAME」に更新<br>「5. アウトプット」節を追加 |
| 2025/08/03 | 今村  | 「エラーコード」節を「エラー一覧」節に更新<br> 「.envファイル」節の「SUBMIT_FILE_NAME」を「SERVER_SUBMIT_FILE_NAME」に更新<br>「5. アウトプット」節を追加 |
...
画像生成のAPI(`/generate`)ではアウトプットとして2枚の画像(印刷用の画像とフロントで表示する用の画像)と指定したパラメータのテキストファイルに書き込む。  

* 格納場所：「`/back/api/app/static/output`」配下の画像ID(`yyyymmddhhmmss_<ランダム4桁>`)フォルダの中
  * 印刷用の画像：`yyyymmddhhmmss_<ランダム4桁>`.png
  * フロントで表示する用の画像：`yyyymmddhhmmss_<ランダム4桁>_display`.png
  * 指定したパラメータのテキストファイル：`yyyymmddhhmmss_<ランダム4桁>`.txt

画像決定のAPI(`/decide`)ではアウトプットとして決定した画像ID(`yyyymmddhhmmss_<ランダム4桁>`)をテキストファイルに書き込む。

* 格納場所：「`/back/api/app/static/output`」配下の「`submit.txt`」
  * .envファイルの「`SERVER_SUBMIT_FILE_NAME`」にて、ファイル名は変更可能である。
...
| エラーコード  | エラーメッセージ                            |
|---------|-------------------------------------|
| E01_001 | ChatGPTの画像生成に失敗しました。                |
| E01_002 | ファイル、もしくはディレクトリが存在しません。             |
| E01_003 | ファイルが既に存在します。                       |
| E01_004 | ファイルへのアクセス権限がありません。                 |
| E01_005 | ファイル操作でエラーが発生しました。                  |
| E01_006 | 画像生成で予期せぬエラーが発生しました。                |
| E02_001 | {xxx}ファイル、もしくはディレクトリが存在しません。        |
| E02_002 | {xxx}ファイルが既に存在します。                  |
| E02_003 | {xxx}ファイルへのアクセス権限がありません。            |
| E02_004 | {xxx}ファイルのファイル操作でエラーが発生しました。        |
| E02_005 | {xxx}ファイルのファイル操作で予期せぬエラーが発生しました。    |
| E02_006 | {xxx}ファイルに重複した画像IDが存在します。(画像ID:yyy) |
